...
import { t as tsParticles } from "./esm-CZKWRZUG.js";
//#region node_modules/react-tsparticles/esm/Utils.js
...
function deepCompare(obj1, obj2, excludeKeyFn = () => false) {
	...
	for (const key of keys1) {
		...
		if (isObject(value1) && isObject(value2)) {
			if (value1 === obj2 && value2 === obj1) continue;
			if (!deepCompare(value1, value2, excludeKeyFn)) return false;
		} else if (Array.isArray(value1) && Array.isArray(value2)) {
			if (!deepCompareArrays(value1, value2, excludeKeyFn)) return false;
...
function deepCompareArrays(arr1, arr2, excludeKeyFn) {
	...
	for (let i = 0; i < arr1.length; i++) {
		...
		if (Array.isArray(val1) && Array.isArray(val2)) {
			if (!deepCompareArrays(val1, val2, excludeKeyFn)) return false;
		} else if (isObject(val1) && isObject(val2)) {
			if (!deepCompare(val1, val2, excludeKeyFn)) return false;
		} else if (val1 !== val2) return false;
	}
	return true;
}
//#endregion
//#region node_modules/react-tsparticles/esm/Particles.js
var defaultId = "tsparticles";
var Particles = class Particles extends import_react.Component {
	constructor(props) {
		super(props);
		this.state = {
			init: false,
	...
	componentDidMount() {
		(async () => {
			if (this.props.init) await this.props.init(tsParticles);
			this.setState({ init: true }, async () => {
	...
	async loadParticles() {
		...
		if (this.props.container) this.props.container.current = container;
		this.setState({ library: container });
		if (this.props.loaded) await this.props.loaded(container);
	}
};
...
//#endregion
//#region node_modules/react-tsparticles/esm/index.js
var esm_default = Particles;
//#endregion
export { Particles, esm_default as default };
