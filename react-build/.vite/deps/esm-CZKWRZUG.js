...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/Vector3d.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/Vector.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Utils/NumberUtils.js
...
function getRandom() {
	return clamp(_random(), 0, .9999999999999999);
}
...
function getParticleDirectionAngle(direction, position, center) {
	if (isNumber(direction)) return direction * Math.PI / 180;
	switch (direction) {
		...
		case "inside": return Math.atan2(center.y - position.y, center.x - position.x);
		case "outside": return Math.atan2(position.y - center.y, position.x - center.x);
		default: return getRandom() * Math.PI * 2;
	}
}
...
function rectBounce(particle, divBounds) {
	const bounds = calculateBounds(particle.getPosition(), particle.getRadius()), resH = rectSideBounce({
		pSide: {
			min: bounds.left,
			max: bounds.right
		},
	...
	if (animationOptions.enable) {
		...
		switch (animationOptions.mode) {
			...
			case "decrease":
				...
				break;
			case "random":
				res.status = getRandom() >= .5 ? "increasing" : "decreasing";
				break;
		}
		...
		switch (animationOptions.startValue) {
			...
			default:
				...
				if (autoStatus) res.status = getRandom() >= .5 ? "increasing" : "decreasing";
				break;
		}
	}
...
function getPositionOrSize(positionOrSize, canvasSize) {
	if (!(positionOrSize.mode === "percent")) {
...
var randomColorValue = "random", midColorValue = "mid", colorManagers = /* @__PURE__ */ new Map();
...
function stringToRgba(input) {
	for (const [, manager] of colorManagers) if (input.startsWith(manager.stringPrefix)) return manager.parseString(input);
	const hexFixed = input.replace(/^#?([a-f\d])([a-f\d])([a-f\d])([a-f\d])?$/i, (_, r, g, b, a) => {
		return r + r + g + g + b + b + (a !== void 0 ? a + a : "");
	}), result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})?$/i.exec(hexFixed);
	return result ? {
...
function rangeColorToRgb(input, index, useIndex = true) {
	...
	if (isString(color.value)) return colorToRgb(color.value, index, useIndex);
	if (isArray(color.value)) return rangeColorToRgb({ value: itemFromArray(color.value, index, useIndex) });
...
function colorToRgb(input, index, useIndex = true) {
	if (!input) return;
	const color = isString(input) ? { value: input } : input;
	if (isString(color.value)) return color.value === randomColorValue ? getRandomRgbColor() : stringToRgb(color.value);
	if (isArray(color.value)) return colorToRgb({ value: itemFromArray(color.value, index, useIndex) });
	for (const [, manager] of colorManagers) {
...
function hslToRgb(hsl) {
	...
	if (!hslPercent.s) result.r = result.g = result.b = hslPercent.l;
	else {
...
function getLinkRandomColor(optColor, blink, consent) {
	...
	if (color === randomColorValue) {
		if (consent) return rangeColorToRgb({ value: color });
		if (blink) return randomColorValue;
		return midColorValue;
	} else if (color === midColorValue) return midColorValue;
	else return rangeColorToRgb({ value: color });
}
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Utils/CanvasUtils.js
...
function drawShapeAfterEffect(container, context, particle, radius, opacity, delta) {
	if (!particle.shape) return;
	const drawer = container.drawers.get(particle.shape);
	if (!drawer || !drawer.afterEffect) return;
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Canvas.js
	...
	constructor(container) {
		...
		this._getPluginParticleColors = (particle) => {
			...
			for (const plugin of this._colorPlugins) {
				if (!fColor && plugin.particleFillColor) fColor = rangeColorToHsl(plugin.particleFillColor(particle));
				if (!sColor && plugin.particleStrokeColor) sColor = rangeColorToHsl(plugin.particleStrokeColor(particle));
				if (fColor && sColor) break;
			}
			return [fColor, sColor];
		};
		this._initCover = () => {
			const cover = this.container.actualOptions.backgroundMask.cover, color = cover.color, coverRgb = rangeColorToRgb(color);
			if (coverRgb) {
				const coverColor = {
					...coverRgb,
					a: cover.opacity
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/BackgroundMask/BackgroundMaskCover.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/BackgroundMask/BackgroundMask.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Interactivity/Events/ClickEvent.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Interactivity/Events/DivEvent.js
var DivEvent = class {
	constructor() {
		this.selectors = [];
		this.enable = false;
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Interactivity/Events/Parallax.js
var Parallax = class {
	constructor() {
		this.enable = false;
		this.force = 2;
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Interactivity/Events/HoverEvent.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Interactivity/Events/ResizeEvent.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Interactivity/Events/Events.js
var Events = class {
	constructor() {
	...
	load(data) {
		...
		if (isBoolean(data.resize)) this.resize.enable = data.resize;
		else this.resize.load(data.resize);
	}
};
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Interactivity/Modes/Modes.js
var Modes = class {
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Interactivity/Interactivity.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/ManualParticle.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Responsive.js
var Responsive = class {
	...
	load(data) {
		if (!data) return;
		if (data.maxWidth !== void 0) this.maxWidth = data.maxWidth;
		if (data.mode !== void 0) if (data.mode === "screen") this.mode = "screen";
		else this.mode = "canvas";
		if (data.options !== void 0) this.options = deepExtend({}, data.options);
	}
};
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Theme/ThemeDefault.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Theme/Theme.js
var Theme = class {
	constructor() {
		this.name = "";
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Collisions/CollisionsAbsorb.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Collisions/CollisionsOverlap.js
...
var RangedAnimationOptions = class extends AnimationOptions {
	...
	load(data) {
		super.load(data);
		if (!data) return;
		if (data.minimumValue !== void 0) this.minimumValue = data.minimumValue;
		if (data.mode !== void 0) this.mode = data.mode;
		if (data.startValue !== void 0) this.startValue = data.startValue;
	}
};
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Random.js
var Random = class {
	constructor() {
		this.enable = false;
		this.minimumValue = 0;
...
var ValueWithRandom = class {
	...
	load(data) {
		if (!data) return;
		if (isBoolean(data.random)) this.random.enable = data.random;
		else this.random.load(data.random);
		if (data.value !== void 0) this.value = setRangeValue(data.value, this.random.enable ? this.random.minimumValue : void 0);
	}
};
//#endregion
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Collisions/Collisions.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Move/MoveAttract.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Move/MoveTrailFill.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Move/MoveTrail.js
var MoveTrail = class {
	constructor() {
		this.enable = false;
		this.length = 10;
	...
	load(data) {
		if (!data) return;
		if (data.enable !== void 0) this.enable = data.enable;
		if (data.fill !== void 0 || data.fillColor !== void 0) this.fill.load(data.fill || { color: data.fillColor });
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Opacity/OpacityAnimation.js
var OpacityAnimation = class extends RangedAnimationOptions {
	...
	load(data) {
		if (data?.opacity_min !== void 0 && data.minimumValue === void 0) data.minimumValue = data.opacity_min;
		super.load(data);
		if (!data) return;
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Opacity/Opacity.js
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Shadow.js
var Shadow = class {
	constructor() {
		this.blur = 0;
		this.color = new OptionsColor();
...
var charKey = "character", charAltKey = "char", imageKey = "image", imageAltKey = "images", polygonKey = "polygon", polygonAltKey = "star";
var Shape = class {
	constructor() {
		this.loadShape = (item, mainKey, altKey, altOverride) => {
			...
			if (mainDifferentValues) this.options[mainKey] = emptyValue;
			if (altDifferentValues && altOverride) this.options[altKey] = emptyValue;
			this.options[mainKey] = deepExtend(this.options[mainKey] ?? emptyValue, item);
			if (!this.options[altKey] || altOverride) this.options[altKey] = deepExtend(this.options[altKey] ?? emptyValue, item);
		};
		this.close = true;
		this.fill = true;
		this.options = {};
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Size/SizeAnimation.js
var SizeAnimation = class extends RangedAnimationOptions {
	...
	load(data) {
		if (data?.size_min !== void 0 && data.minimumValue === void 0) data.minimumValue = data.size_min;
		super.load(data);
		if (!data) return;
		if (data.destroy !== void 0) this.destroy = data.destroy;
	}
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Options/Classes/Particles/Size/Size.js
...
var ParticlesOptions = class {
	...
	load(data) {
		...
		if (data.groups !== void 0) for (const group in data.groups) {
		...
		if (collisions !== void 0) this.collisions.enable = collisions;
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/InteractionManager.js
...
var Particle = class {
	constructor(engine, id, container, position, overrideOptions, group) {
		...
		this._getRollColor = (color) => {
			...
			const backFactor = this.roll.horizontal && this.roll.vertical ? 2 : 1, backSum = this.roll.horizontal ? Math.PI / 2 : 0;
	...
	destroy(override) {
		...
		for (const [, plugin] of container.plugins) if (plugin.particleDestroyed) plugin.particleDestroyed(this, override);
		for (const updater of container.particles.updaters) if (updater.particleDestroyed) updater.particleDestroyed(this, override);
		if (pathGenerator) pathGenerator.reset(this);
	}
	draw(delta) {
	...
	init(id, position, overrideOptions, group) {
		...
		if (drawer && drawer.loadShape) drawer.loadShape(this);
		const sideCountFunc = drawer?.getSidesCount;
		if (sideCountFunc) this.sides = sideCountFunc(this);
		this.spawning = false;
		this.shadowColor = rangeColorToRgb(this.options.shadow.color);
		for (const updater of container.particles.updaters) updater.init(this);
		for (const mover of container.particles.movers) mover.init && mover.init(this);
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/Point.js
var Point = class {
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/Range.js
var Range = class {
	constructor(x, y) {
		this.position = {
			x,
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/Rectangle.js
var Rectangle = class Rectangle extends Range {
	...
	intersects(range) {
		if (range instanceof Circle) range.intersects(this);
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/Circle.js
var Circle = class Circle extends Range {
	...
	intersects(range) {
		...
		if (range instanceof Circle) return r + range.radius > Math.sqrt(distPos.x ** 2 + distPos.y ** 2);
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/QuadTree.js
var QuadTree = class QuadTree {
	constructor(rectangle, capacity) {
		...
		this._subdivide = () => {
			...
			for (let i = 0; i < 4; i++) this._subs.push(new QuadTree(new Rectangle(x + width / 2 * (i % 2), y + height / 2 * (Math.round(i / 2) - i % 2), width / 2, height / 2), capacity));
			this._divided = true;
		};
		this._points = [];
		this._divided = false;
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Particles.js
var qTreeCapacity = 4;
...
var Particles = class {
	constructor(engine, container) {
		this._applyDensity = (options, manualCount, group) => {
			if (!options.number.density?.enable) return;
		...
		this._pushParticle = (position, overrideOptions, group, initializer) => {
			try {
				...
				else particle = new Particle(this._engine, this._nextId, this._container, position, overrideOptions, group);
				let canAdd = true;
	...
	addManualParticles() {
		...
		for (const particle of options.manualParticles) this.addParticle(particle.position ? getPosition(particle.position, container.canvas.size) : void 0, particle.options);
	}
	addParticle(position, overrideOptions, group, initializer) {
		const limit = this._container.actualOptions.particles.number.limit;
		if (limit > 0) {
	...
	init() {
		...
		for (const [, plugin] of container.plugins) {
			if (plugin.particlesInitialization !== void 0) handled = plugin.particlesInitialization();
			if (handled) break;
		}
		this._interactionManager.init();
		...
		if (!handled) {
			for (const group in options.particles.groups) {
				...
				for (let i = this.count, j = 0; j < groupOptions.number?.value && i < options.particles.number.value; i++, j++) this.addParticle(void 0, groupOptions, group);
			}
	...
	async update(delta) {
		...
		for (const [, plugin] of container.plugins) plugin.update && plugin.update(delta);
...
var defaultPathGeneratorKey = "default", defaultPathGenerator = {
...
var Container = class {
	constructor(engine, id, sourceOptions) {
		...
		if (typeof IntersectionObserver !== "undefined" && IntersectionObserver) this._intersectionObserver = new IntersectionObserver((entries) => this._intersectionManager(entries));
	...
	destroy() {
		...
		this.canvas.destroy();
		for (const [, drawer] of this.drawers) drawer.destroy && drawer.destroy(this);
	...
	async init() {
		...
		for (const [, drawer] of this.drawers) drawer.init && await drawer.init(this);
	...
	setPath(pathOrGenerator, init, update) {
		...
		if (isFunction(pathOrGenerator)) {
			...
			if (init) pathGenerator.init = init;
			if (update) pathGenerator.update = update;
		} else {
	...
	updateActualOptions() {
		...
		if (this.responsiveMaxWidth === newMaxWidth) return false;
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Core/Utils/Plugins.js
...
var Engine = class {
	...
	addConfig(nameOrConfig, config) {
		...
		} else this._configs.set(nameOrConfig.name ?? "default", nameOrConfig);
	}
	addEventListener(type, listener) {
	...
	async addShape(shape, drawer, initOrRefresh, afterEffectOrRefresh, destroyOrRefresh, refresh = true) {
		...
		if (isBoolean(initOrRefresh)) {
			...
			realInit = void 0;
		} else realInit = initOrRefresh;
		if (isBoolean(afterEffectOrRefresh)) {
			realRefresh = afterEffectOrRefresh;
			realAfterEffect = void 0;
		} else realAfterEffect = afterEffectOrRefresh;
		if (isBoolean(destroyOrRefresh)) {
			realRefresh = destroyOrRefresh;
			realDestroy = void 0;
		} else realDestroy = destroyOrRefresh;
		if (isFunction(drawer)) customDrawer = {
	...
	async loadFromArray(tagIdOrOptionsOrParams, optionsOrIndex, index) {
		...
		if (!isParams(tagIdOrOptionsOrParams)) {
			params = {};
			if (isString(tagIdOrOptionsOrParams)) params.id = tagIdOrOptionsOrParams;
			else params.options = tagIdOrOptionsOrParams;
			if (isNumber(optionsOrIndex)) params.index = optionsOrIndex;
			else params.options = optionsOrIndex ?? params.options;
			params.index = index ?? params.index;
		} else params = tagIdOrOptionsOrParams;
		return this._loadParams(params);
	}
	async loadJSON(tagId, pathConfigJson, index) {
		let url, id;
		if (isNumber(pathConfigJson) || pathConfigJson === void 0) url = tagId;
		else {
			id = tagId;
			url = pathConfigJson;
		}
	...
	async set(id, element, options, index) {
		const params = { index };
		if (isString(id)) params.id = id;
		else params.element = id;
		if (element instanceof HTMLElement) params.element = element;
		else params.options = element;
		if (isNumber(options)) params.index = options;
		else params.options = options ?? params.options;
		return this._loadParams(params);
	}
	...
	setOnClickHandler(callback) {
		const dom = this.dom();
		if (!dom.length) throw new Error(`${errorPrefix} can only set click handlers after calling tsParticles.load()`);
		for (const domItem of dom) domItem.addClickHandler(callback);
	}
	async _loadParams(params) {
		const id = params.id ?? `tsparticles${Math.floor(getRandom() * 1e4)}`, { index, url } = params, options = url ? await getDataFromUrl({
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Utils/HslColorManager.js
var HslColorManager = class {
	...
	parseString(input) {
		...
		const result = /hsla?\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(,\s*([\d.%]+)\s*)?\)/i.exec(input);
...
//#endregion
//#region node_modules/tsparticles-engine/esm/Utils/RgbColorManager.js
var RgbColorManager = class {
	...
	parseString(input) {
		...
		const result = /rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(,\s*([\d.%]+)\s*)?\)/i.exec(input);
		return result ? {
			a: result.length > 4 ? parseAlpha(result[5]) : 1,
...
//#endregion
//#region node_modules/tsparticles-engine/esm/init.js
function init() {
...
//#endregion
export { isSsr as A, getRandom as B, getLogger as C, isInArray as D, isDivModeEnabled as E, addEasing as F, setRangeValue as G, getRangeValue as H, clamp as I, mouseLeaveEvent as J, Vector as K, getDistance as L, itemFromSingleOrMultiple as M, loadFont as N, isObject as O, rectBounce as P, getDistances as R, executeOnSingleOrMultiple as S, isArray as T, getValue as U, getRangeMax as V, randomInRange as W, mouseMoveEvent as Y, calculateBounds as _, OptionsColor as a, divMode as b, colorMix as c, getLinkRandomColor as d, getStyleFromHsl as f, rgbToHsl as g, rangeColorToRgb as h, ValueWithRandom as i, itemFromArray as j, isPointInside as k, getHslAnimationFromHsl as l, rangeColorToHsl as m, Circle as n, drawLine as o, getStyleFromRgb as p, errorPrefix as q, Rectangle as r, drawTriangle as s, tsParticles as t, getLinkColor as u, circleBounce as v, initParticleNumericAnimationValue as w, divModeExecute as x, circleBounceDataFromParticle as y, getEasing as z };

//# sourceMappingURL=esm-CZKWRZUG.js.map